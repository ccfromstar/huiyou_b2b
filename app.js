...
var permission = require('./routes/permission.js');
...
nodeUtil.log(permission.config.session.store);
app.use(session({
...
var sched = later.parse.recur().every(1).second(),
    t = later.setInterval(function() {
        var d = new Date();
        //set to run every day at 0:00:00
        if(d.getHours() == 0 && d.getHours() == 0 && d.getMinutes() == 0 && d.getSeconds() == 0){
            schedule.checkBookingstatus();
        }
        //other time to run...
    }, sched);
...
var server = http.createServer(app).listen(httpsPort);
