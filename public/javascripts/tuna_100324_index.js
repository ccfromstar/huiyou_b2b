...
$extend(String.prototype, {
    ...
    isChinaIDCard: function () {
        ...
        if (this.match(/^\d{17}[\dx]$/i)) {
            for (var b = 0, c = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2], d = 0; d < 17; d++) b += parseInt(a[d], 10) * c[d];
    ...
    parseStdDate: function (a) {
        ...
        $$.status.version == "en" && (b = b.replace(/[a-z]{3,}/i, function (a) {
...
function $removeTextNode(a) {
    if (a) {
        for (var b = a.firstChild, c; b;) c = b.nextSibling,
...
$$.support = {
    testCss: function () {
        var a = __.body,
            b = $c("div"),
            c, d, f, g = parseFloat($(a).$getStyle("marginTop")) || 0;
...
$offset = function (a) {
    ...
    for (var j = $$.support, p = /^t(?:able|d|h)$/i;
...
Ctrip.module.recommend = function (a) {
    function b(a) {
        ...
        f in j ? (g.innerHTML = j[f], eval(m)) : (g.innerHTML = '<div class="pic_loading"></div>', $ajax(a, null, function (a) {
...
Ctrip.module.allyes = function (a) {
    ...
    setTimeout(function () {
        ...
        a.innerHTML = '<iframe marginheight="0" width="100%" height="100%" marginwidth="0" frameborder="0" scrolling="no" src=""></iframe>'.replace("{$user}", d);
        else {
            var b = [];
            f && (b = f.map(function (a) {
                a.button = a.button || ";";
                return '<div class="base_ad140x60" style="height:{$height}px">{$iframe}</div>'.replace("{$height}", a.height).replace("{$iframe}", '<iframe marginheight="0" width="100%" height="100%" marginwidth="0" frameborder="0" scrolling="no" src=""></iframe>'.replace("{$user}", a.user))
            }));
...
Ctrip.module.notice = function (a) {
    ...
    90);
    a.form && ($(a.form).$r("submit", function () {
        if (a.isNull()) a.value = "";
        setTimeout(function () {
    ...
    a.$r("click", function () {
        (function (a) {
            for (var b = 0; b < a.childNodes.length; b++) with(a.childNodes[b]) if (nodeType == 3) {
                ...
                nodeValue = nodeValue.replace(f, function (a) {
...
(function (a) {
    ...
    b.prototype = {
        ...
        fillContent: function (a, b) {
            ...
            return a.replace(RegExp(c, "gi"), function (a, c, d, f, g, h, m) {
...
(function () {
    ...
    Ctrip.module.address = function (k) {
        ...
        function S() {
            ...
            for (b in f.hotData) {
                var c = [],
                    c = f.hotData[b].replace(/@([^@]*)\|([^@]*)/g, function (a, b, c) {
        ...
        function V() {
            ...
            if (f.hotData) {
                ...
                var a = f.hotTemplate.container.replaceWith({
                    ...
                    b = "",
                    a = a.replace(/\{guestId:(\w+)\}/g, function (a, c) {
            ...
            f.hotTarget = u;
                ...
                if (a) {
                    ...
                    var c = $$.module.address.source[o.source],
                        ...
                        n = [],
                        j = [],
                        c = c.replace(RegExp("@([^\\|@]*\\|)?" + a + "[^@]*", "gi"), function (a) {
                        h.push(a);
                        return ""
                    });
                    h && h.sort(b);
        ...
        o.hook = {};
        (k.getAttribute("mod_address_hook") || "").replace(/(on)?([^;:]+):([^;]+)/gi, function (a, b, c, d) {
        ...
        o.check = function () {
            ...
            if (a = a.replace(/([\(\)\\\[\]\.\+\?\*\|\^\$])/gi, "\\$1").replace(/@|\|/gi, "")) {
                ...
                g = [],
                w = [];
                c = c.replace(o.auto ? RegExp("@([^\\|@]*\\|)?" + a + "[^@]*", "gi") : RegExp("@([^\\|@]*\\|)?" + a + "(\\|[^@]*)?(?=@)", "gi"), function (a) {
                    v.push(a);
                    return ""
                });
                v && v.sort(b);
                ...
                g && g.sort(b);
                o.auto && (c = c.replace(f, function (a) {
                    w.push(a);
                    return ""
                }), w && w.sort(b));
...
(function (a) {
    ...
    function c(a) {
        if (!a.$getData("__inited__")) {
            a.$r("focus", f);
            a.$r("blur", g);
            a.addEventListener ? a.addEventListener("input", j, !1) : a.attachEvent("onpropertychange", function () {
                e = window.event;
    ...
    b.prototype = {
        ...
        handleMousedown: function (a) {
            ...
            (a = a.$parentNode("dl")) && (c = a.getAttribute("t").replace("dd", d));
            b.value = c;
            this.setDateNote();
            this._elem.blur();
            this.focusNext();
