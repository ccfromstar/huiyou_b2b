...
function createPagination(){
    ...
    parent.ResizeIframeFromParent('page2');
}
        ...
        if (product[i].status_id == 3 || product[i].status_id == 4) {
            ...
            for (var j in pp) {
                if (pp[j].product_id == product[i].id) {
                    if (pp[j].retail_price < dp && pp[j].retail_price != 0) {
                        dp = pp[j].retail_price;
                    }
                }
            }
            ...
            for (var j in pp) {
                if (pp[j].product_id == product[i].id && pp[j].cabin_type_id == 1) {
                    if (pp[j].retail_price < dp1 && pp[j].retail_price != 0) {
                        dp1 = pp[j].retail_price;
                        dp2 = pp[j].amount + "间";
                    }
                }
                if (pp[j].product_id == product[i].id && pp[j].cabin_type_id == 2) {
                    if (pp[j].retail_price < dp3 && pp[j].retail_price != 0) {
                        dp3 = pp[j].retail_price;
                        dp4 = pp[j].amount + "间";
                    }
                }
                if (pp[j].product_id == product[i].id && pp[j].cabin_type_id == 3) {
                    if (pp[j].retail_price < dp5 && pp[j].retail_price != 0) {
                        dp5 = pp[j].retail_price;
                        dp6 = pp[j].amount + "间";
                    }
                }
                if (pp[j].product_id == product[i].id && pp[j].cabin_type_id == 4) {
                    if (pp[j].retail_price < dp7 && pp[j].retail_price != 0) {
                        dp7 = pp[j].retail_price;
                        dp8 = pp[j].amount + "间";
                    }
                }
            }
            ...
            html =
                ...
                '<td rowspan="5" style="width: 100px;text-align: center;line-height: 20px;"><span style="font-size: 12px;color: #000000;">最低结算价</span><br/><span style="font-size: 16px;color: #FA8723;font-weight: bold">￥' + dp + '</span><br/><br/><br/>';
            if (product[i].status_id == 3) {
                //if (product[i].owner_id != parent.document.getElementById('current-user-id')) {
                    html += '&nbsp;<img onclick="parent.openBooking(\'' + product[i].product_number + '\');" style="cursor:pointer;margin-left: 0px;" src="/images/booking2.png" /><br/>';
                //}
            }
            html += '<img onclick="maskIframe(\'加载中...\',false);window.location=\'productdetail?p=' + product[i].product_number + '\';" style="cursor:pointer;margin-left: 0px;" src="/images/detail.png" /></td></tr>' +
            ...
            '<tr style="text-align: center"><td style="width: 76px;">内舱结算</td><td style="width: 76px;">海景结算</td><td style="width: 76px;">阳台结算</td><td style="width: 76px;">套房结算</td><td style="width: 76px;">港务费</td><td style="width: 76px;">岸上游</td><td style="width: 76px;">邮轮小费</td><td style="width: 76px;">领队费</td><td style="width: 76px;">签证费</td><td style="width: 76px;">保险费</td></tr>' +
            ...
            html +=
                '</tr><tr><td colspan="10" style="width: 760px;line-height: 20px;height: 40px;padding:10px">' + adv + '</td></tr>' +
                '</table>';
